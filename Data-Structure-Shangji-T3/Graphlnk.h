...
template<class T, class E>
void Prim(Graphlnk<T, E> &G, const T u0, E * dist, int * path) {
	int i;
	...
	int u = G.getVertexPos(u0);				// 求起始顶点号u

	edge<T, E> * eds = new edge<T, E>[n];
	edge<T, E> ed;
	edge<T, E> * ed_job;

	MinHeap<E, edge<T, E> * > H(m);			// 最小堆，关键码类型为E

	bool * Vmst = new bool[n];				// 最小生成树顶点集合，判断是否已经加入顶点集合
	for (i = 0; i < n; i++) {				// 数组初始化，初始值都为false
		Vmst[i] = false;
	}

	Vmst[u] = true;							// u加入生成树
	count = count2 = 1;						// 顶点集合已有元素数+1
	...
	do {
		...
		while (v != -1) {
			...
			if (Vmst[v] == false) {
				...
				ed_job->head = v;
				ed_job->tail = u;
				ed_job->data = G.getWeight(u, v);
				H.Insert(ed_job);

				count2++;
			}
			v = G.getNextNeighbor(u, v);	// 找顶点u的下一个邻接顶点v(格式：u有邻接点v1, v2, ..)
		}

		count2 = 1;		// 重置 count2

		while (H.isEmpty() == false && count < n) {
			H.removeMin(ed_job);				// 从堆中退出具最小权值的边ed
			if (Vmst[ed_job->head] == false) {
				path[count] = ed_job->head;
...
template<class T, class E>
void Graphlnk<T, E>::printShortestPath(Graphlnk<T, E> &G, int v, int v2, E * dist, int * path) {
	int i;
	int j;
	int k;
	int n = G.NumberOfVertices();
...
void printPrimPath(int * path) {
	int length = _msize(path) / sizeof(int);
